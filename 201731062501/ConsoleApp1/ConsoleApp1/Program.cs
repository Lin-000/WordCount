...
namespace ConsoleApp1
{
    public class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            string str = String.Format("字符总数：{0}\r\n单词总数：{1}\r\n总的行数：{2}\r\n", ch_num, word, nr); // 构造要输出的信息字符串
            StringBuilder sb = new StringBuilder();
            Dictionary<string, int> group = CountPhrase.WordsGroups(wordList, m);
            foreach (string s in d.Keys)
            {
                sb.Append(string.Format("单词：{0}，出现的频率：{1}\r\n", s, d[s]));
            }

            if (flag == true)
            {
                foreach (string s in group.Keys)
                {
                    sb.Append(string.Format("词组：{0}，出现的频率：{1}\r\n", s, group[s]));
                }
            }
